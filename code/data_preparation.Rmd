...
read.csv(paste0(wd,"/../data/original/",batch,"/index_",batch,".csv")) %>%
  ...
  rename(
    ...
    library_PCR_cycles_given=Number.of.PCR.cycles,
    #indexing stats
...
read.csv(paste0(wd,"/../data/original/",batch,"/preprocessing_",batch,".csv")) %>%
  ...
  select(
    ...
    #host genome info
    Reference.genome,Reference.genome.species,Reference.genome.closeness,
...
### Reference genomes
```{r mags_prep, echo=FALSE}
read.csv(paste0(wd,"/../data/original/",batch,"/genomes_",batch,".csv")) %>%
  #select relevant columns
  select(
    #data identifiers
    Code,Accession,
  ...
  #save clean table
  write.csv(paste0(wd,"/../data/genomes.csv"), row.names = FALSE)
```
...
ref <- read_delim("../data/genomes.csv")
...
combined <- ppr %>%
  inner_join(., ref, by = "reference_id", keep = FALSE) %>%
  ...
  mutate(singlem_fraction = as.numeric(str_replace(singlem_fraction, "%", ""))/100,
         ...
         bp_survived_fastp = bases_post_fastp / bases_pre_fastp,
         reads_survived_fastp = reads_post_fastp / reads_pre_fastp,
         tax_group = case_when(
